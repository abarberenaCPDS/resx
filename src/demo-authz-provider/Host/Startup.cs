...
namespace Host
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var p1 = Configuration.GetSection("Policy");
            var p2 = Configuration.GetSection("PoliciesSrc");
            var p3 = Configuration.GetSection("Policies2");

            // this sets up the PolicyServer client library and policy provider - configuration is loaded from appsettings.json
            services.AddPolicyServerClient(p1)
                //services.AddPolicyServerClient(p2)
